...
1. 문제를 틀린다
2. `_템플릿/오답노트 템플릿.md` 를 복사해 `오답노트/` 에 붙여넣는다
3. **"왜 그렇게 생각했나"** 를 먼저 채운다 — 정답 코드보다 이게 자산이다
4. 에러가 났으면 `에러/` 에 해당 노트가 있는지 보고, 없으면 만들고, 있으면 사례를 추가한다
5. 판단을 놓친 거라면 `개념정리/` 에 기준을 적는다. 개념 자체가 부족했으면 `교과서/` 에 노트를 만든다
6. 코드는 `코드/문제이름.py` 로 저장. `if __name__ == "__main__":` 아래에 테스트를 붙여 `python 코드/문제이름.py` 로 바로 검증
...
- **위키링크 사용**: 켬
- **모든 파일 확장자 감지**: 켬 — `.py` 파일이 탐색기에 보인다

### 플러그인 (선택)

| 플러그인 | 왜 |
|---|---|
| **Obsidian Git** | 노트를 저장할 때마다 자동 커밋·푸시. 이 저장소를 GitHub과 계속 동기화하는 데 가장 편하다 |
| **Dataview** | `tags` frontmatter로 "Lv.2 오답노트만 모아보기" 같은 목록을 자동 생성 |
| **Templater** | `_템플릿/` 을 단축키로 삽입, 날짜 자동 입력 |

셋 다 없어도 구조는 그대로 돌아간다. Obsidian Git 하나만 먼저 깔아도 충분하다.
