...
> [!important] 코드는 `.py` 파일 하나가 원본이다
> 오답노트 안에 풀이 전체를 복붙하지 않는다. 같은 코드가 두 군데 있으면 반드시 어긋난다.
> 노트에는 **틀린 줄과 고친 줄**만 남기고, 전체 코드는 `코드/*.py` 를 링크한다.
> 옵시디언에서 그 링크를 클릭하면 **VSCode Editor** 플러그인이 편집기로 열어준다 (아래 참고).
