...
정사각형이면 **터지지도 않는다.** 에러 없이 조용히 틀린 답이 나온다.

실제 사례: [[공원산책]]

### 6. 음수 인덱스는 애초에 터지지 않는다

```python
lst[-1]       # 에러가 아니라 맨 뒤 원소
```

격자에서 왼쪽·위로 범위를 벗어나면 인덱스가 음수가 되는데, 파이썬은 이를 **반대쪽 끝**으로 읽는다.
오른쪽·아래로 벗어나면 `IndexError` 로 알려 주지만 **왼쪽·위는 알려 주지 않는다.**
`0 <= i < len(lst)` 처럼 **아래쪽 경계도 직접** 검사해야 한다.
